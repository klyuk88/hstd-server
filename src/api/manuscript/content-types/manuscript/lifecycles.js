module.exports = {
    ...
    async afterUpdate(event) {
        ...
        for (let index = 0; index < entry.authors.length; index++) {
            ...
            if (author.corresponding && result.statusWork.slug === 'revise') {
                strapi.service('api::manuscript.manuscript').sendMail(author.email, locale === 'ru' ? reviseMessage.ru : reviseMessage.en)

            } else if (author.corresponding && result.statusWork.slug === 'reject') {
                strapi.service('api::manuscript.manuscript').sendMail(author.email, locale === 'ru' ? rejectMessage.ru : rejectMessage.en)
            } else if (author.corresponding && result.statusWork.slug === 'accept') {

                strapi.service('api::manuscript.manuscript').sendMail(author.email, locale === 'ru' ? acceptMessage.ru : acceptMessage.en)
            }
